...
def test_label_crowns_basic_success():
    ...
    crowns = Vector(gpd.GeoDataFrame(
    ...
    result = loaders.label_crowns(crowns, points, label_col='species_label', max_dist=1.0)
    
    assert 'species' in result.columns
    assert result.data.iloc[0]['species'] == 'Oak'
...
def test_label_crowns_max_distance_logic():
    ...
    result = loaders.label_crowns(crowns, points, label_col='species_label', max_dist=2.0)
...
def test_label_crowns_crs_mismatch_reprojection():
    ...
    result = loaders.label_crowns(crowns, points, label_col='species_label')
...
def test_label_crowns_validates_missing_column():
    ...
    with pytest.raises(ValueError, match="not found in source points"):
        loaders.label_crowns(crowns, points, label_col='species_label')
...
def test_label_crowns_deduplication():
    ...
    assert len(result) == 1
    assert result.data.index.is_unique